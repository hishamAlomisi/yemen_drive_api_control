...
IF NOT EXISTS (
    SELECT * FROM [__EFMigrationsHistory]
    WHERE [MigrationId] = N'20260912191548_Stage3PaymentIdempotency'
)
BEGIN
    ALTER TABLE [PaymentTransactions] ADD [IdempotencyKey] nvarchar(128) NULL;
END;
...
BEGIN
    EXEC(N'CREATE UNIQUE INDEX [IX_PaymentTransactions_UserId_IdempotencyKey] ON [PaymentTransactions] ([UserId], [IdempotencyKey]) WHERE [IdempotencyKey] IS NOT NULL');
END;
...
BEGIN
    INSERT INTO [__EFMigrationsHistory] ([MigrationId], [ProductVersion])
    VALUES (N'20260912191548_Stage3PaymentIdempotency', N'9.0.9');
END;
